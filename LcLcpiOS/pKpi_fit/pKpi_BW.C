...
void pKpi_BW() {
    ...
    RooBreitWigner bw("bw", "bw", m, mean1, sigma1);
    ...
    frame->SetTitle("pKpi Breit Wigner Fit; m{pK#pi} (MeV/c^{2}); Entries");
    data.plotOn(frame); 
    model.plotOn(frame, RooFit::Components("bkg"), RooFit::LineStyle(kDashed), RooFit::LineColor(kRed));
    model.plotOn(frame, RooFit::Components("bw"), RooFit::LineStyle(kDashed), RooFit::LineColor(kGreen));
