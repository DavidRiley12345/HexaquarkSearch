...
void sw_allvar_makesig_M(){
   ROOT::RDataFrame d("weightedtree","/home/ppe/d/driley/LcLcpiOS/Sweight/weighted.root");
   ...
   d_new.Snapshot("weightedtree","/home/ppe/d/driley/LcLcpiOS/Sweight/allvars/weighted.root");
} 
...
{ 
  ...
  TFile* fin = TFile::Open("/home/ppe/d/driley/LcLcpiOS/Sweight/allvars/weighted.root"); 
  ...
  double maxVals[nVars] = {3000,         2360,       450000,         30000,         220000,           16000,        125000,           13000,          140000,           140000, 100000,   6000,  10000,   10000,  220000,         30000,         30000,    450000,7000 };

  // Loop over variables
  for (int i = 0; i < nVars; i++) {
    TH1D *h_sig = new TH1D("h_sig", ("Sweighted " + vars[i]).c_str(), 40, minVals[i], maxVals[i]);
    TH1D *h_bkg = new TH1D("h_bkg", ("Bweighted " + vars[i]).c_str(), 40, minVals[i], maxVals[i]);

    TCanvas* cdata = new TCanvas("","",800,800);
    cdata->cd(1);
    tree->Draw((vars[i] + ">>h_sig").c_str(), "nsig_sw");
    tree->Draw((vars[i] + ">>h_bkg").c_str(), "nbkg_sw");
